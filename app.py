...
# - Simplified sidebar (date range only)
# - Liga logo header
# - Two-column chart layout per country
# - CPI YoY (US), IPCA 12m (BR)
# - Combined CSV (last 12 months, month-end aligned)
# - PIB (BR) shown in R$ milhões in Overview
...
# -----------------------------
# Constants (default series IDs)
# -----------------------------
FRED_IDS = {
    "fedfunds": "FEDFUNDS",     # effective fed funds rate
    "cpi_index": "CPIAUCSL",    # CPI (1982-84=100)
    "gdp": "GDP",               # nominal GDP, billions USD (quarterly)
    "unemp": "UNRATE",
    "retail": "RSAFS",
}

# BCB SGS daily/meta & monthly series (we use daily Selic meta: 432; IPCA m/m %: 433)
SGS_IDS = {
    "selic_target_daily": 432,  # meta Selic (diário)
    "ipca_mom_pct": 433,        # IPCA var. mensal (%)
    "retail_index": 1552,       # PMC volume index
}

# World Bank indicators (annual)
WB_IND_BRA = {
    "gdp_current_lcu": "NY.GDP.MKTP.CN",
    "unemp_pct": "SL.UEM.TOTL.ZS",
}
...
def fetch_fred(series_id: str, start: Optional[str], end: Optional[str]) -> pd.DataFrame:
    ...
    Priority:
      1) Official FRED API (requires free API key) if provided (st.secrets['FRED_API_KEY']).
...
def fetch_bcb_sgs(series_id: int, start: Optional[str], end: Optional[str],
    ...
    for i in range(tries):
        try:
            ...
            rc = requests.get(
                url_csv,
                headers={"Accept": "text/csv, */*;q=0.1", "User-Agent": UA_HEADERS["User-Agent"]},
                timeout=timeout,
            )
            rc.raise_for_status()
            txt = rc.text.strip()
...
def fetch_worldbank(indicator: str, country_code: str = "USA", start_year: int = 1990, end_year: Optional[int] = None) -> pd.DataFrame:
    """Fetch World Bank series (annual)."""
    try:
...
def px_line_common(df: pd.DataFrame, title: str, y_label: str):
    fig = px.line(df.reset_index(), x="date", y="value", title=title, template="plotly_dark")
    fig.update_layout(
...
def yoy_rate_from_index(df: pd.DataFrame) -> pd.DataFrame:
    """12-month % change from a price index level."""
...
def combo_two_series(df1: pd.DataFrame, label1: str, df2: pd.DataFrame, label2: str, title: str):
    """Overlay two % series on the same axis."""
...
# -----------------------------
# Sidebar (simplified: only dates)
# -----------------------------
st.sidebar.header("Settings")
st.sidebar.markdown("Select the date range for the data displayed:")

col1, col2 = st.sidebar.columns(2)
with col1:
    start_date = st.date_input("Start", value=date(2010, 1, 1), key="start_date")
with col2:
...
with st.spinner("Fetching data..."):
    # USA (FRED)
    df_fedfunds = fetch_fred(FRED_IDS["fedfunds"], start_str, end_str)
    df_cpi_us = fetch_fred(FRED_IDS["cpi_index"], start_str, end_str)
    df_gdp_us = fetch_fred(FRED_IDS["gdp"], start_str, end_str)
    df_unrate_us = fetch_fred(FRED_IDS["unemp"], start_str, end_str)
    df_retail_us = fetch_fred(FRED_IDS["retail"], start_str, end_str)

    # Brazil (SGS + World Bank)
    df_selic = fetch_bcb_sgs(SGS_IDS["selic_target_daily"], start_str, end_str)
    df_ipca = fetch_bcb_sgs(SGS_IDS["ipca_mom_pct"], start_str, end_str)
    df_retail_br = fetch_bcb_sgs(SGS_IDS["retail_index"], start_str, end_str)

    df_gdp_br = fetch_worldbank(WB_IND_BRA["gdp_current_lcu"], "BRA",
                                start_year=pd.to_datetime(start_str).year,
                                end_year=pd.to_datetime(end_str).year)
    df_unemp_br = fetch_worldbank(WB_IND_BRA["unemp_pct"], "BRA",
                                  start_year=pd.to_datetime(start_str).year,
                                  end_year=pd.to_datetime(end_str).year)
...
# Layout
# -----------------------------
tabs = st.tabs(["Overview", "USA", "Brazil", "Downloads"])
...
with tabs[1]:
    ...
    with col2:
        fig_us_combo = combo_two_series(df_fedfunds, "Fed Funds (%)", df_cpi_us_yoy, "CPI YoY (%)",
                                        "US – Rates vs Inflation (YoY %)")
        if fig_us_combo:
...
with tabs[2]:
    ...
    with col2:
        fig_br_combo = combo_two_series(df_selic, "Selic (%)", df_ipca_12m, "IPCA 12m (%)",
                                        "Brasil – Selic vs IPCA 12m (YoY %)")
        if fig_br_combo:
...
with tabs[3]:
    ...
    st.write("Download clean CSVs for each series, or a combined file aligned by month-end for the last 12 months.")

    labeled = {
        "fedfunds_usa": df_fedfunds.rename(columns={"value": "fedfunds_usa"}),
        "cpi_usa_yoy": df_cpi_us_yoy.rename(columns={"value": "cpi_usa_yoy"}),
        ...
        "ipca_bra_12m": df_ipca_12m.rename(columns={"value": "ipca_bra_12m"}),
    ...
    # Combined monthly last 12 months
    st.markdown("---")
    st.markdown("**Combined CSV (last 12 months, month-end)**")

    def _pick_numeric_col(df: pd.DataFrame, preferred: str) -> Optional[str]:
    ...
    def _monthly_last12(df: pd.DataFrame, preferred_col: str) -> pd.Series:
        ...
        sm = s.resample("M").last().ffill().dropna()
        if len(sm) > 12:
            sm = sm.iloc[-12:]
...
with st.expander("ℹ️ Sources & tips"):
    st.markdown(
        """
- **USA (FRED)** via official API (if key provided via Streamlit secrets `FRED_API_KEY`) or public **fredgraph.csv** fallback.
...
- The app caches results. Change the dates to refresh the fetch.
"""
    )
# Placeholder for Streamlit app (app.py content from previous message)
